...
export function NamingResultsPage() {
  ...
  useEffect(() => {
    if (!missionId) return;
    (async () => {
      const { data, error: fetchErr } = await fetchMissionRow(missionId, 'id, questions, naming_candidates, naming_criteria, naming_test_type, brand_personality, aggregated_by_question, status, title, goal_type, completed_at, qualified_respondent_count, analysis');
      if (fetchErr || !data) {
