...
export function CompetitorAnalysisResultsPage() {
  ...
  useEffect(() => {
    if (!missionId) return;
    (async () => {
      const { data, error: fetchErr } = await fetchMissionRow(missionId, 'id, questions, brand_name, category, competitor_brands, attribute_battery, aggregated_by_question, status, title, goal_type, completed_at, qualified_respondent_count, analysis');
      if (fetchErr || !data) {
  ...
  // (`!focal` only happens when the mission row is absent — kept as crash guard + type narrowing.)
  if (!focal || (mission?.status !== 'completed' && brands.length === 0)) {
    return (
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] text-[var(--t1)]">
      ...
      />
      {/* Pass 48 — grounded results copilot */}
      <ChatWidget scope="results" missionId={missionId} suggestions={resultsChatSuggestions('competitor')} />
    </div>
  );
}
