...
export function ValidateResultsPage() {
  ...
  useEffect(() => {
    if (!missionId) return;
    (async () => {
      const { data, error: fetchErr } = await fetchMissionRow(missionId, 'id, questions, brand_name, category, concept_description, concept_media_url, concept_media_type, concept_price_usd, aggregated_by_question, analysis, status, title, goal_type, completed_at, qualified_respondent_count');
      if (fetchErr || !data) {
