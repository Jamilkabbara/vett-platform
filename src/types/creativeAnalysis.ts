...
/**
 * The chosen placement and the deterministic comparison against its published
 * attention norm. predicted and delta are null when the run returned no usable
 * prediction; the comparison is withheld, never guessed.
 */
export interface PlacementBenchmark {
  placement_id: string;
  placement_label: string;
  norm_active_seconds: number;
  predicted_active_seconds: number | null;
  delta_vs_norm_pct: number | null;
}

/** Qualitative market notes. Never numbers: the backend drops any item with a digit. */
export interface MarketContext {
  cultural_fit?: string[];
  localisation_risks?: string[];
...
export interface CreativeAnalysisV2 extends CreativeAnalysisV1 {
  ...
  /** Present when the customer chose a placement. */
  placement_benchmark?: PlacementBenchmark;
  /** Present when the customer chose a market. */
  market?: { code: string; name: string };
  /** Present when a market was chosen and its notes were produced. */
